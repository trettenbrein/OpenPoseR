...
#
# Compute velocity for points on y-axis for given data frame in OpenPoseR format

velocity_y <- function(data, model, fps = 25) {
  ...
  velocity_y <- data.frame(matrix(nrow = 0, ncol = num_cols))
  velocity_y <- setNames(velocity_y, points_y)

  # Compute velocity (y-axis) for all points in model
  for(i in 1:(nrow(data)-1)){
    # Formula: ((pixel at t1)-(pixel at t1-1))/((t1)-(t1-1))
    velocity_y <- rbind(velocity_y, (data[i+1,points_y] -
                                       data[i,points_y])/k)
  }
