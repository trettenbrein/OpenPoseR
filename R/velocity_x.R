...
velocity_x <- function(data, model, fps = 25) {
  ...
  velocity_x <- data.frame(matrix(nrow = 0, ncol = num_cols))
  velocity_x <- setNames(velocity_x, points_x)

  # Compute velocity (x-axis) for all points in model
  for(i in 1:(nrow(data)-1)){
    # Formula: ((pixel at t1)-(pixel at t1-1))/((t1)-(t1-1))
    velocity_x <- rbind(velocity_x, (data[i+1,points_x] -
                                       data[i,points_x])/k)
  }
